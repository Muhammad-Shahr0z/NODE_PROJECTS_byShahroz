...
do {
    ...
    if (Selectios.select === "Student") {
        const ask = await inquirer.prompt({
            type: "input",
            name: "name",
            message: chalk.cyanBright.bold("Enter The Student Name You Want To Connect With :"),
        ...
        if (checkStudent) {
            ...
            for (let i = 0; i < studentsArray.length; i++) {
                console.log(chalk.green(`\t${i + 1}. ${studentsArray[i].name.charAt(0).toUpperCase() +
    ...
    else if (Selectios.select === "View Student List") {
