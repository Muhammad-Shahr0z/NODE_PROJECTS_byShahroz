...
class accountHolder {
    ...
    accountNumber;
    accountBalance;
    constructor(fullName, age, gender, phone, email, password, accountNumber, accountBalance) {
...
let baseAccountDigits = 1000;
let baseIban = "IBAN"; // Generate Random Account Number To assign Account Holder
let allAccounts = []; // main array for all account holders 
let condition = true; // loop condition for loop running
let atmCharges = 1500; // ATM charges Deduction If account Successfully Created
/*******************************************************************/
let currentUserFname; // 01  Get value from inquirer
let currentAge; // 02  Get value from inquirer
let currentGender; // 03  Get value from inquirer
let currentPhone; // 04  Get value from inquirer
let currentEmail; // 05  Get value from inquirer
let currentPassword; // 06  Get value from inquirer
let currentBalance = 0; // 07  Get value from inquirer
let RandomAccount; // 08  Get value From Assign value
do { //// Loop Start ///
    ...
    if (selectoperation.selectOperation === "Create Account") {
        ...
        let createdAccount = new accountHolder(currentUserFname, currentAge, currentGender, currentPhone, currentEmail, currentPassword, RandomAccount, currentBalance);
