...
class STUDENT {
    ...
    feeAmount;
    studentNumber;
    constructor(id, name, age, coursesEntrolled, feeAmount, studentNumber) {
        this.id = id;
        this.name = name;
        this.age = age;
...
do {
    ...
    if (selectAction.ans === "Enroll a Student") {
        ...
        if (checkStudent.includes(inputName) === false) {
            ...
            studentCount++;
            let Studentarrays = new STUDENT(GenrateId, inputName, studentName.age, CoursesSelect.ans, studentFee, studentCount);
            ...
            students.forEach((student) => {
                if (student.name === StudentnameList.ans) {
                    let titleCase = student.name
                        ...
                        .join(" ");
                    console.log(chalk.yellow.bold(`\t\t\t\t\t\t\t\t\t      Student ${student.studentNumber} Status`));
                    console.log(chalk.green.bold("\t\t\t\t\t\t\t\t    ************************************"));
                    console.log(chalk.magentaBright.bold(`\t\t\t\t\t\t\t\t\t       ${titleCase}              `));
                    console.log(chalk.green.bold("\t\t\t\t\t\t\t\t    ************************************"));
                    console.log(chalk.green.bold("\t\t\t\t\t\t\t\t\tStudent Name: ") +
                        chalk.yellowBright(titleCase));
                    console.log(chalk.green.bold("\t\t\t\t\t\t\t\t\tStudent Age: ") +
                        chalk.yellowBright(student.age));
                    console.log(chalk.green.bold("\t\t\t\t\t\t\t\t\tStudent Course : ") +
                        chalk.yellowBright(student.coursesEntrolled));
                    console.log(chalk.green.bold("\t\t\t\t\t\t\t\t\tStudent Fee: ") +
                        chalk.yellowBright(student.feeAmount));
                    console.log(chalk.green.bold("\t\t\t\t\t\t\t\t\tStudent ID: ") +
                        chalk.yellowBright(student.id));
                    console.log(chalk.green.bold("\t\t\t\t\t\t\t\t\tStudent Remaining Balance: ") +
        ...
        console.log(chalk.green.bold("\t\t\t\t\t\t\t\t\tCurrent Balance: ") +
